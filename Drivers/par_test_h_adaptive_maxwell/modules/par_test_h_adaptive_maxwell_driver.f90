...
!========================================================================================
  ...
  subroutine write_solution(this)
    ...
    if(this%test_params%get_write_solution()) then
      if (this%par_environment%am_i_l1_task()) then
        ...
        cell_vector = 0.0_rp 
         ! Fill cell_set_ids with identifiers for cells touching selected dofs 
         fe_loop: do while ( .not. fe%has_finished())
           if ( fe%is_local() ) then 
           call fe%get_fe_dofs(fe_dofs) 
           do idof=1, fe%get_num_dofs() 
           if ( 1709 < (fe_dofs(1)%p(idof)) .and. (fe_dofs(1)%p(idof)) < 1722 ) then 
           cell_vector(fe%get_gid()) = 1.0_rp 
           call fe%next; cycle fe_loop
           end if 
           end do 
           end if 
           call fe%next() 
  ...
  function par_test_h_adaptive_maxwell_driver_popcorn_fun(point,num_dim) result (val)
    ...
    real(rp)          :: domain(6)
    real(rp)          :: inner_region_lx, inner_region_ly, inner_region_lz
    real(rp)          :: lx, ly, lz 

    environment => this%triangulation%get_environment()
    if ( environment%am_i_l1_task() ) then
       inner_region_size = this%test_params%get_inner_region_size() 
       inner_region_lx = inner_region_size(0) 
       inner_region_ly = inner_region_size(1) 
       inner_region_lz = inner_region_size(2) 
          ...
          if ( cell%is_local() ) then
             if ( cell%get_level() < this%test_params%get_min_num_refinements() ) then
                ...
                do inode=1,cell%get_num_nodes() 
                   x = cell_coordinates(inode)%get(1) 
                   y = cell_coordinates(inode)%get(2) 
                   z = cell_coordinates(inode)%get(3) 
                   if ( ( ((lx-inner_region_lx)/2.0_rp<=x) .and. (x<=(lx+inner_region_lx)/2.0_rp) ) .and. &
                        ( ((ly-inner_region_ly)/2.0_rp<=y) .and. (y<=(ly+inner_region_ly)/2.0_rp) ) .and. & 
                        ( ((lz-inner_region_lz)/2.0_rp<=z) .and. (z<=(lz+inner_region_lz)/2.0_rp) ) ) then

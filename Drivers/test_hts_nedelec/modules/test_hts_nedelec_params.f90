...
contains
  ...
  subroutine hts_nedelec_params_define_parameters(this)
    ...
    error = list%set(key = static_triang_generate_from_key , value =  static_triang_generate_from_struct_hex_mesh_generator) ; check(error==0)
    ...
    ! MESHING parameters 
    error = switches%set(key = struct_hex_mesh_generator_num_dims_key      , value = '--num_dims') ; check(error==0)
    error = switches%set(key = struct_hex_triang_num_cells_dir   , value = '--num_cells')   ; check(error==0)
    error = switches%set(key = struct_hex_mesh_generator_is_dir_periodic_key           , value = '--is_dir_periodic')   ; check(error==0)
    ...
    error = switches_ab%set(key = struct_hex_mesh_generator_num_dims_key   , value = '-dim')       ; check(error==0)
    error = switches_ab%set(key = struct_hex_triang_num_cells_dir, value = '-nelem')     ; check(error==0)
    error = switches_ab%set(key = struct_hex_mesh_generator_is_dir_periodic_key        , value = '-idp')       ; check(error==0)
    ...
    ! MESHING parameters
    error = helpers%set(key = struct_hex_mesh_generator_num_dims_key   , value = 'Number of dimensions')  ; check(error==0)
    error = helpers%set(key = struct_hex_triang_num_cells_dir, value = 'Number of elements in each direction'); check(error==0)
    error = helpers%set(key = struct_hex_mesh_generator_is_dir_periodic_key        , value = 'Is periodic? in each direction'); check(error==0)
    ...
    error = required%set(key = struct_hex_mesh_generator_is_dir_periodic_key        , value = .false.)  ; check(error==0)

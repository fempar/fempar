...
contains
  ...
  subroutine par_pb_bddc_poisson_params_define_parameters(this)
    ...
    error = list%set(key = num_parts_per_dir_key       , value =  [4,4,0,2,2,0,1,1,0]) ; check(error==0)
  ...
  function get_nparts(this)
    ...
    assert(error==0)       
    error = list%GetShape(key = num_parts_per_dir_key   , shape = array_size); 
    check(error==0)
    assert(array_size(1) >= num_levels*SPACE_DIM)
    call memalloc(array_size(1), num_parts_per_dir)
    error = list%get(key = num_parts_per_dir_key , value = num_parts_per_dir) 
    check(error==0)
    get_nparts=num_parts_per_dir(1:3)
    if (allocated(array_size)) deallocate(array_size) 
    call memfree(num_parts_per_dir)

...
contains
  ...
  subroutine set_cells_for_refinement(this)
    ...
    integer(ip) :: istat, k
				...
				real(rp), parameter :: y0 = 0.45_rp
				real(rp), parameter :: xL = 0.55_rp
				real(rp), parameter :: yL = 0.55_rp 
				real(rp), parameter :: z0 = 0.45_rp 
				real(rp), parameter :: zL = 0.55_rp 
    integer(ip), parameter :: max_level = 4

    call this%triangulation%create_cell_iterator(cell)
    if (this%triangulation%get_num_dims() == 2) then
      allocate(coords(4),stat=istat); check(istat==0)
      ...
      deallocate(coords,stat=istat); check(istat==0)
				 ...
				 allocate(coords(8),stat=istat); check(istat==0)
      ...
      do while ( .not. cell%has_finished() )
        ...
        !do k=1,8
								...
								!						 ((z0 < coords(k)%get(3)) .and. (coords(k)%get(3) < zL)))      &
								!				    .and. (cell%get_level()<= max_level) .or. (cell%get_level() == 0) )then
        !  call cell%set_for_refinement(); exit 
        !end if
        !end do
    ...
    type is (hex_lagrangian_reference_fe_t)
       ...
       h_refinement_subedge_permutation  => reference_fe%get_h_refinement_subedget_permutation()
				type is (hex_nedelec_reference_fe_t) 
				   h_refinement_interpolation        => reference_fe%get_h_refinement_interpolation()
       h_refinement_subcell_permutation  => reference_fe%get_h_refinement_subcell_permutation()
    class default
      assert(.false.)
    end select
  ...
  subroutine setup_fe_space(this)
    ...
    integer(ip) :: set_ids_to_reference_fes(1,2)
      ...
      call this%vector_poisson_conditions%set_boundary_function_Hx(this%vector_poisson_analytical_functions%get_boundary_function_Hx()) 
						call this%vector_poisson_conditions%set_boundary_function_Hy(this%vector_poisson_analytical_functions%get_boundary_function_Hy()) 
						call this%vector_poisson_conditions%set_boundary_function_Hz(this%vector_poisson_analytical_functions%get_boundary_function_Hz()) 
      ...
      elseif ( this%test_params%get_fe_formulation() == 'dG' ) then

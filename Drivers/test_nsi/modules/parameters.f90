...
contains
  ...
  subroutine set_mesh_group_lists(this)
    ...
    error = values_group%set(key = static_triang_generate_from_key, value = static_triang_generate_from_struct_hex_mesh_generator); check(error==0)
    ! Domain definition
    ...
    error = values_group%set(key = struct_hex_mesh_generator_is_dir_periodic_key , value = [non_periodic,non_periodic,non_periodic]); check(error==0)
    ! Mesh characterization
    error = values_group%set(key = reference_fe_geo_order_key   , value = 1);                         check(error==0)
    error = values_group%set(key = struct_hex_mesh_generator_num_dims_key     , value = 2);                         check(error==0)
    ...
    error = values_group%set(key = struct_hex_mesh_generator_num_levels_key       , value = 1);       check(error==0)
    error = values_group%set(key = struct_hex_mesh_generator_num_parts_x_dim_key, value = [1,1,0]); check(error==0)
    
    ! Switches
    ! --------
    ! Structured/non-structured
    error = switches_group%set(key = static_triang_generate_from_key, value = '--triangulation-type'); check(error==0)
    ! Domain definition
    ...
    error = switches_group%set(key = struct_hex_mesh_generator_is_dir_periodic_key , value = '--periodicity')  ; check(error==0)
    ! Mesh characterization
    error = switches_group%set(key = reference_fe_geo_order_key   , value = '--reference-fe-geo-order'); check(error==0)
    error = switches_group%set(key = struct_hex_mesh_generator_num_dims_key     , value = '--dim')                   ; check(error==0)
    ...
    ! Mesh parallelization
    error = switches_group%set(key = struct_hex_mesh_generator_num_levels_key       , value = '--num_levels'); check(error==0)
    error = switches_group%set(key = struct_hex_mesh_generator_num_parts_x_dim_key, value = '--num_parts') ; check(error==0)
    
    ! Switches abbreviation
    ! ---------------------
    ! Structured/non-structured
    error = switches_ab_group%set(key = static_triang_generate_from_key, value = '-tt'); check(error==0)
    ! Domain definition
    ...
    error = switches_ab_group%set(key = struct_hex_mesh_generator_is_dir_periodic_key , value = '-period'); check(error==0)
    ! Mesh characterization
    error = switches_ab_group%set(key = reference_fe_geo_order_key   , value = '-gorder'); check(error==0)
    error = switches_ab_group%set(key = struct_hex_mesh_generator_num_dims_key     , value = '-dm')    ; check(error==0)
    ...
    ! Mesh parallelization
    error = switches_ab_group%set(key = struct_hex_mesh_generator_num_levels_key       , value = '-l') ; check(error==0)
    error = switches_ab_group%set(key = struct_hex_mesh_generator_num_parts_x_dim_key, value = '-np'); check(error==0)
    ...
    error = helpers_group%set(key = static_triang_generate_from_key, value = msg); check(error==0)
    ! Domain definition
    ...
    error = helpers_group%set(key = struct_hex_mesh_generator_is_dir_periodic_key, value = 'Domain periodicity per direction'); check(error==0)
    ! Mesh characterization
    ...
    error = helpers_group%set(key = struct_hex_mesh_generator_num_dims_key   , value = 'Number of space dimensions')                         ; check(error==0)
    ...
    ! Mesh parallelization
    error = helpers_group%set(key = struct_hex_mesh_generator_num_levels_key       , value = 'Number of parallel solver coarsening levels'); check(error==0)
    error = helpers_group%set(key = struct_hex_mesh_generator_num_parts_x_dim_key, value = 'Number of parts per direction and per level'); check(error==0)

    ! Required
    ! --------
    ! Structured/non-structured
    error = required_group%set(key = static_triang_generate_from_key, value = .false.); check(error==0)
    ! Domain definition
    ...
    error = required_group%set(key = struct_hex_mesh_generator_is_dir_periodic_key , value = .false.); check(error==0)
    ! Mesh characterization
    error = required_group%set(key = reference_fe_geo_order_key   , value = .false.); check(error==0)
    error = required_group%set(key = struct_hex_mesh_generator_num_dims_key     , value = .false.); check(error==0)
    ...
    ! Mesh parallelization
    error = required_group%set(key = struct_hex_mesh_generator_num_levels_key       , value = .false.); check(error==0)
    error = required_group%set(key = struct_hex_mesh_generator_num_parts_x_dim_key, value = .false.); check(error==0)
  ...
  function get_triangulation_type(this)
    ...
    assert(error==0)
    assert(values_group%isAssignable(static_triang_generate_from_key, get_triangulation_type))
    error = values_group%Get(key = static_triang_generate_from_key, Value = get_triangulation_type)
    assert(error==0)

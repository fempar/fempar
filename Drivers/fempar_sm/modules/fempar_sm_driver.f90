...
# include "debug.i90"
  ...
  type fempar_sm_fe_driver_t 
     ...
     type(fempar_sm_CG_discrete_integration_t) :: fempar_sm_integration
...
!#endif  
     ...
     ! Fempar_sm problem solution FE function
...
!========================================================================================
  ...
  subroutine setup_coarse_fe_handlers(this)
    ...
    integer(ip) :: istat
    allocate(this%coarse_fe_handlers(2), stat=istat)
    check(istat==0)
    this%coarse_fe_handlers(1)%p => this%coarse_fe_handler
    this%coarse_fe_handlers(2)%p => this%coarse_fe_handler
...
!========================================================================================
  ...
  subroutine setup_fe_space(this)
    ...
    integer(ip) ::  number_fields,number_components

    number_fields = 2
    if ( this%par_environment%am_i_l1_task() ) then
       number_components = 0
       reference_fe => this%reference_fes(1)%p
       number_components = number_components + reference_fe%get_number_field_components()
       reference_fe => this%reference_fes(2)%p
       number_components = number_components + reference_fe%get_number_field_components()
       assert(number_components == 3)
    else
       number_components = 3
    end if

    call memalloc(number_fields,field_blocks,__FILE__,__LINE__)
    call memalloc(number_fields,number_fields,field_coupling,__FILE__,__LINE__)
    field_blocks   = (/1,1/)
    field_coupling = reshape((/.true.,  .false.,  &
            &                  .false.,  .true.   /),(/2,2/))    
    
    call this%fempar_sm_conditions%set_number_components(number_components)    
    ...
    call this%fempar_sm_conditions%set_boundary_function(this%fempar_sm_analytical_functions%get_boundary_function())
    call this%fe_space%interpolate_dirichlet_values(this%fempar_sm_conditions)    
    !call this%fe_space%print()
    
    call memfree(field_blocks,__FILE__,__LINE__)
    call memfree(field_coupling,__FILE__,__LINE__) 
  ...
  subroutine setup_system (this)
    ...
    call this%fempar_sm_integration%set_analytical_functions(this%fempar_sm_analytical_functions)
  ...
  subroutine check_solution(this)
    ...
    mean      = vector_error_norm%compute(this%fempar_sm_analytical_functions%get_vector_solution_function(), this%solution, mean_norm)   
    l1        = vector_error_norm%compute(this%fempar_sm_analytical_functions%get_vector_solution_function(), this%solution, l1_norm)   
    l2        = vector_error_norm%compute(this%fempar_sm_analytical_functions%get_vector_solution_function(), this%solution, l2_norm)   
    lp        = vector_error_norm%compute(this%fempar_sm_analytical_functions%get_vector_solution_function(), this%solution, lp_norm)   
    linfty    = vector_error_norm%compute(this%fempar_sm_analytical_functions%get_vector_solution_function(), this%solution, linfty_norm)   
    h1_s      = vector_error_norm%compute(this%fempar_sm_analytical_functions%get_vector_solution_function(), this%solution, h1_seminorm) 
    h1        = vector_error_norm%compute(this%fempar_sm_analytical_functions%get_vector_solution_function(), this%solution, h1_norm) 
    w1p_s     = vector_error_norm%compute(this%fempar_sm_analytical_functions%get_vector_solution_function(), this%solution, w1p_seminorm)   
    w1p       = vector_error_norm%compute(this%fempar_sm_analytical_functions%get_vector_solution_function(), this%solution, w1p_norm)   
    w1infty_s = vector_error_norm%compute(this%fempar_sm_analytical_functions%get_vector_solution_function(), this%solution, w1infty_seminorm) 
    w1infty   = vector_error_norm%compute(this%fempar_sm_analytical_functions%get_vector_solution_function(), this%solution, w1infty_norm)  
    ...
    mean      = scalar_error_norm%compute(this%fempar_sm_analytical_functions%get_scalar_solution_function(), this%solution, mean_norm)   
    l1        = scalar_error_norm%compute(this%fempar_sm_analytical_functions%get_scalar_solution_function(), this%solution, l1_norm)   
    l2        = scalar_error_norm%compute(this%fempar_sm_analytical_functions%get_scalar_solution_function(), this%solution, l2_norm)   
    lp        = scalar_error_norm%compute(this%fempar_sm_analytical_functions%get_scalar_solution_function(), this%solution, lp_norm)   
    linfty    = scalar_error_norm%compute(this%fempar_sm_analytical_functions%get_scalar_solution_function(), this%solution, linfty_norm)   
    h1_s      = scalar_error_norm%compute(this%fempar_sm_analytical_functions%get_scalar_solution_function(), this%solution, h1_seminorm) 
    h1        = scalar_error_norm%compute(this%fempar_sm_analytical_functions%get_scalar_solution_function(), this%solution, h1_norm) 
    w1p_s     = scalar_error_norm%compute(this%fempar_sm_analytical_functions%get_scalar_solution_function(), this%solution, w1p_seminorm)   
    w1p       = scalar_error_norm%compute(this%fempar_sm_analytical_functions%get_scalar_solution_function(), this%solution, w1p_norm)   
    w1infty_s = scalar_error_norm%compute(this%fempar_sm_analytical_functions%get_scalar_solution_function(), this%solution, w1infty_seminorm) 
    w1infty   = scalar_error_norm%compute(this%fempar_sm_analytical_functions%get_scalar_solution_function(), this%solution, w1infty_norm)  
    if ( this%par_environment%am_i_l1_root() ) then
  ...
  subroutine write_solution(this)
    implicit none
    class(fempar_sm_fe_driver_t), intent(in) :: this
    type(output_handler_t)                          :: oh
    if(this%test_params%get_write_solution()) then
       if ( this%par_environment%am_i_l1_task() ) then
        call oh%create()
        call oh%attach_fe_space(this%fe_space)
        call oh%add_fe_function(this%solution, 1, 'solution1')
        call oh%add_fe_function(this%solution, 2, 'solution2')

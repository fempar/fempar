...
# include "debug.i90"
  ...
  type, abstract :: base_sparse_matrix_t
   ...
   contains
     ...
     procedure, public :: apply_body                       => base_sparse_matrix_apply_body
     procedure, public :: apply_add_body                   => base_sparse_matrix_apply_add_body
     procedure, public :: apply_transpose_body             => base_sparse_matrix_apply_transpose_body
     procedure, public :: apply_transpose_add_body         => base_sparse_matrix_apply_transpose_add_body
     procedure, public :: apply_to_dense_matrix_body       => base_sparse_matrix_apply_to_dense_matrix_body
     procedure, public :: apply_transpose_to_dense_matrix_body => base_sparse_matrix_apply_transpose_to_dense_matrix_body
  ...
  !---------------------------------------------------------------------
    ...
    private
        ...
        procedure, public :: apply_to_dense_matrix_body              => coo_sparse_matrix_apply_to_dense_matrix_body
        procedure, public :: apply_transpose_to_dense_matrix_body    => coo_sparse_matrix_apply_transpose_to_dense_matrix_body
...
!---------------------------------------------------------------------
   ...
   subroutine base_sparse_matrix_append_coords_by_col_body(this, nz, ia, ja) 
    ...
    !-----------------------------------------------------------------
        check(.false.)
    end subroutine base_sparse_matrix_apply_transpose_add_body
...
!---------------------------------------------------------------------
    ...
    subroutine coo_sparse_matrix_add(this, alpha, op1, beta, op2)
    !-----------------------------------------------------------------
    ...
    !-----------------------------------------------------------------
        ...
        type(coo_sparse_matrix_t) :: tmp1, tmp2
    !-----------------------------------------------------------------
        ...
        class is (coo_sparse_matrix_t) 
           ...
           class is (coo_sparse_matrix_t) ! op1 and op2 are COO
              call coo_sparse_matrix_add_coo( this, alpha, op1, beta, op2 )
           class DEFAULT 
              call op2%copy_to_coo_body( tmp2 )
              call coo_sparse_matrix_add_coo( this, alpha, op1, beta, tmp2 )
              call tmp2%free()
           end select
        class DEFAULT
           select type(op2)
           class is (coo_sparse_matrix_t) ! op1 and op2 are COO
              call op1%copy_to_coo_body( tmp1 )
              call coo_sparse_matrix_add_coo( this, alpha, tmp1, beta, op2 )
              call tmp1%free()
           class DEFAULT 
              call op1%copy_to_coo_body( tmp1 )
              call op2%copy_to_coo_body( tmp2 )
              call coo_sparse_matrix_add_coo( this, alpha, tmp1, beta, tmp2 )
              call tmp1%free()
              call tmp2%free()
           end select
        end select
    end subroutine coo_sparse_matrix_add 
    
    subroutine coo_sparse_matrix_add_coo(this, alpha, op1, beta, op2)
    !-----------------------------------------------------------------
    !< Sum COO values this = alpha*op1 + beta*op2
    !-----------------------------------------------------------------
        type(coo_sparse_matrix_t),  intent(inout)  :: this
        real(rp),                    intent(in)    :: alpha
        type(coo_sparse_matrix_t),   intent(in)    :: op1
        real(rp),                    intent(in)    :: beta
        type(coo_sparse_matrix_t) ,  intent(in)    :: op2
    !----------------------------------------------------------------- 
         assert( op1%state_is_assembled() .and. op2%state_is_assembled() )
         ...
         if ( this%state_is_assembled() ) then
           massert( op1%get_nnz() == this%get_nnz(), 'coo_sparse_matrix_add :: op1 and this must have the same sparsity pattern' ) 
           massert( all( op1%ia(1:op1%get_nnz()) ==  this%ia(1:this%get_nnz()) ), 'coo_sparse_matrix_add :: op1 and this must have the same sparsity pattern' )

...
abstract interface
  subroutine fe_get_elem2dof_interface ( this, elem2dof, n )
    import :: SB_finite_element_t, p_array_ip1_t, ip
    implicit none
...
abstract interface
  ...
  subroutine fe_get_volume_integrator_interface ( this, volume_integrator, n )
    import :: SB_finite_element_t, p_array_ip1_t, ip, SB_p_volume_integrator_t
    implicit none
    class(SB_finite_element_t), target, intent(in)  :: this
    integer(ip), intent(in) :: n
...
integer(ip), allocatable :: bc_value(:)
contains
...
public :: SB_finite_element_t, SB_simple_finite_element_t
...
!    class(SB_p_fe_space_t), allocatable :: fe_space_array(:)  
!    integer(ip) :: field_block(:)

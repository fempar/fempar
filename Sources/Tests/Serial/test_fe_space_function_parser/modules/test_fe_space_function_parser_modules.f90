...
contains  
  ...
  subroutine source_term_get_value_space ( this, point, result )
    ...
    if ( this%get_num_dims() == 2 ) then  ! -((6*x*y^3)+(6*x^3*y))
      result = -(( 6*point%get(1)*point%get(2)**3 ) + & 
                 ( 6*point%get(1)**3*point%get(2) )) 
    else if ( this%get_num_dims() == 3 ) then ! -((6*x*y^3*z^3)+(6*x^3*y*z^3)+(6*x^3*y^3*z))
      result = -(( 6*point%get(1)*point%get(2)**3*point%get(3)**3 ) + & 
                 ( 6*point%get(1)**3*point%get(2)*point%get(3)**3 ) + & 
                 ( 6*point%get(1)**3*point%get(2)**3*point%get(3) ))  
    end if
  ...
  subroutine boundary_function_get_value_space ( this, point, result )
    ...
    if ( this%get_num_dims() == 2 ) then
      result = point%get(1)**3*point%get(2)**3 ! x^3*y^3 
    else if ( this%get_num_dims() == 3 ) then
      result = point%get(1)**3*point%get(2)**3*point%get(3)**3 ! x^3*y^3*z^3
    end if  
  ...
  subroutine solution_function_get_gradient_space ( this, point, result )
    ...
    assert ( this%get_num_dims() == 2 .or. this%get_num_dims() == 3 )
    if ( this%get_num_dims() == 2 ) then
      call result%set( 1, 3*point%get(1)**2*point%get(2)**3 ) ! 3*x^2*y^3 
      call result%set( 2, 3*point%get(1)**3*point%get(2)**2 ) ! 3*x^3*y^2 
    else if ( this%get_num_dims() == 3 ) then
      call result%set( 1, 3*point%get(1)**2*point%get(2)**3*point%get(3)**3 ) ! 3*x^2*y^3*z^3
      call result%set( 2, 3*point%get(1)**3*point%get(2)**2*point%get(3)**3 ) ! 3*x^3*y^2*z^3
      call result%set( 3, 3*point%get(1)**3*point%get(2)**3*point%get(3)**2 ) ! 3*x^3*y^3*z^2
    end if

...
!*   In the following code, we will use
  ...
  contains
    subroutine test_stokes_parameter_list_define_fempar_parameters(this)
      ...
      !* Manage a new fempar parameter
      call this%add(key='viscosity', switch= '--VISCOSITY', Value=1.0, help= 'Value of the viscosity')

      !* Update default FE space fempar parameter values
      call this%update(key=fes_num_fields_key,          value=2)
      call this%update(key=fes_num_ref_fes_key,         value=2)
      call this%update(key=fes_ref_fe_types_key,        value='Lagrangian Lagrangian')
      call this%update(key=fes_ref_fe_orders_key,       value=[2, 1])
      call this%update(key=fes_ref_fe_conformities_key, value=[.true., .true.])
      call this%update(key=fes_ref_fe_continuities_key, value=[.true., .false.])
      call this%update(key=fes_field_types_key,         value=field_type_vector // " " // field_type_scalar)
      call this%update(key=fes_field_blocks_key,        value=[1, 1])

      !* We want to get the root of the operator, i.e., solve Ax = b. 
      !* We are going to use an iterative solver. 
      !* We overwrite the default values in the parameter list for tolerance,
      !* type of solver (Conjugate Gradient) and max number of iterations.
      call this%update(key=ils_rtol_key,               value=1.0e-12_rp)
      call this%update(key=ils_max_num_iterations_key, value=5000)
      call this%update(key=ils_type_key,               value=fgmres_name)

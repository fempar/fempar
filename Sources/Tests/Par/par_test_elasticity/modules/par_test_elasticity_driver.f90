...
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
module test_elasticity_driver_names
  use fempar_names
  use test_elasticity_params_names
  use test_elasticity_discrete_integration_names
  use test_elasticity_conditions_names
  use test_elasticity_analytical_functions_names
  use test_elasticity_linear_solver_names
  use test_elasticity_nonlinear_operator_names
  use test_elasticity_nonlinear_solver_names
...
# include "debug.i90"
  ...
  type test_elasticity_fe_driver_t 
     private 
     
     ! Place-holder for parameter-value set provided through command-line interface
     type(test_elasticity_params_t)       :: test_params
     ...
     class(test_elasticity_discrete_integration_t), allocatable :: test_elasticity_integration
     type(test_elasticity_conditions_t)                :: test_elasticity_conditions
     type(test_elasticity_analytical_functions_t)      :: test_elasticity_analytical_functions
  ...
  end type test_elasticity_fe_driver_t

  ! Types
  public :: test_elasticity_fe_driver_t

contains

  subroutine run_simulation(this) 
    implicit none
    class(test_elasticity_fe_driver_t), intent(inout) :: this
    ...
    unknown => this%solution%get_free_dof_values()
    !write(*,*) unknown%nrm2()
...
!========================================================================================
  ...
  subroutine setup_discrete_integration(this)
    ...
    if ( this%test_params%get_discrete_integration_type() == discrete_integration_type_irreducible ) then
       allocate(irreducible_discrete_integration_t :: this%test_elasticity_integration, stat=istat); check(istat==0)
    else if(this%test_params%get_discrete_integration_type() == discrete_integration_type_mixed_u_p ) then
       allocate(mixed_u_p_discrete_integration_t :: this%test_elasticity_integration, stat=istat); check(istat==0)
   else
   ...
   call this%test_elasticity_integration%create(this%triangulation%get_num_dims(),this%test_elasticity_analytical_functions)
   !call this%test_elasticity_integration%set_solution(this%solution)
...
!========================================================================================
  subroutine setup_reference_fes(this)
    ...
    allocate(this%reference_fes(this%test_elasticity_integration%get_number_fields()), stat=istat)
    check(istat==0)
    
    if ( this%par_environment%am_i_l1_task() ) then
      call this%triangulation%create_cell_iterator(cell)
      reference_fe_geo => cell%get_reference_fe()
      do field_id = 1, this%test_elasticity_integration%get_number_fields()
         this%reference_fes(field_id) =  make_reference_fe ( topology = reference_fe_geo%get_topology(), &
                                                             fe_type = this%test_elasticity_integration%get_fe_type(field_id), &
                                                             num_dims = this%triangulation%get_num_dims(), &
                                                             order = this%test_params%get_reference_fe_order(), &
                                                             field_type = this%test_elasticity_integration%get_field_type(field_id), &
                                                             conformity = .true. )
      end do      
...
!========================================================================================
  ...
  subroutine setup_coarse_fe_handlers(this)
    implicit none
    class(test_elasticity_fe_driver_t), target, intent(inout) :: this
    integer(ip) :: istat, field_id
    allocate(this%coarse_fe_handlers(this%test_elasticity_integration%get_number_fields()), stat=istat); check(istat==0)
    do field_id = 1, this%test_elasticity_integration%get_number_fields()
...
!========================================================================================
  ...
  subroutine setup_fe_space(this)
    ...
    call this%test_elasticity_analytical_functions%set_num_dimensions(this%triangulation%get_num_dims())
    call this%test_elasticity_conditions%set_number_components(this%test_elasticity_integration%get_number_components())
    call this%test_elasticity_conditions%set_number_dimensions(this%triangulation%get_num_dims())    
    ! Store exact function to define B.C. in FE space
    call this%test_elasticity_conditions%set_boundary_function(this%test_elasticity_analytical_functions%get_solution_function_u())
    call this%fe_space%create( triangulation       = this%triangulation, &
                               conditions          = this%test_elasticity_conditions, &
  ...
  subroutine setup_operators (this)
    ...
    ! FE operator
    if(this%test_elasticity_integration%is_symmetric().and.this%test_elasticity_integration%is_coercive()) then
       call this%fe_affine_operator%create ( sparse_matrix_storage_format      = csr_format, &
            &                                diagonal_blocks_symmetric_storage = [ .true. ], &
            &                                diagonal_blocks_symmetric         = [ .true. ], &
            &                                diagonal_blocks_sign              = [ SPARSE_MATRIX_SIGN_POSITIVE_DEFINITE ], &
            &                                fe_space                          = this%fe_space, &
            &                                discrete_integration              = this%test_elasticity_integration, &
            &                                field_blocks                      = this%test_elasticity_integration%get_field_blocks(),            &
            &                                field_coupling                    = this%test_elasticity_integration%get_field_coupling()) 
    else if(this%test_elasticity_integration%is_symmetric()) then
       call this%fe_affine_operator%create ( sparse_matrix_storage_format      = csr_format, &
            &                                diagonal_blocks_symmetric_storage = [ .true. ], &
            &                                diagonal_blocks_symmetric         = [ .true. ], &
    ...
    do field_id = 1, this%test_elasticity_integration%get_number_fields()
       if(this%test_elasticity_integration%get_field_type(field_id) == field_type_vector) then
         call this%fe_space%interpolate(field_id, this%zero_vector, this%solution)
       else if( this%test_elasticity_integration%get_field_type(field_id) == field_type_scalar) then
         call this%fe_space%interpolate(field_id, this%zero_scalar, this%solution)
       end if
    end do
    call this%fe_space%interpolate_dirichlet_values(this%solution)    
    call this%test_elasticity_integration%set_fe_function(this%solution)
    ...
    if(this%test_elasticity_integration%is_coercive()) then
    ...
    ! Nonlinear solver ! test_elasticity_abs_res_norm_and_rel_inc_norm
    call this%nonlinear_solver%create(convergence_criteria = test_elasticity_abs_res_norm, & 
         &                                         abs_tol = 1.0e-6,  &
         &                                         rel_tol = 1.0e-9, &
         &                                       max_iters = 10   ,  &
  ...
  subroutine check_solution(this)
    ...
    do field_id = 1, this%test_elasticity_integration%get_number_fields()
       ...
       if(this%test_elasticity_integration%get_field_type(field_id) == field_type_vector) then
          ...
          mean      = vector_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_u(), this%solution, mean_norm)   
          l1        = vector_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_u(), this%solution, l1_norm)   
          l2        = vector_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_u(), this%solution, l2_norm)   
          lp        = vector_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_u(), this%solution, lp_norm)   
          linfty    = vector_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_u(), this%solution, linfty_norm)   
          h1_s      = vector_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_u(), this%solution, h1_seminorm) 
          h1        = vector_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_u(), this%solution, h1_norm) 
          w1p_s     = vector_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_u(), this%solution, w1p_seminorm)   
          w1p       = vector_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_u(), this%solution, w1p_norm)   
          w1infty_s = vector_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_u(), this%solution, w1infty_seminorm) 
          w1infty   = vector_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_u(), this%solution, w1infty_norm)  
          if ( this%par_environment%am_i_l1_root() ) then
             write(*,'(a12,a8)')      this%test_elasticity_integration%get_field_name(field_id), ' field: '
          ...
          mean      = scalar_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_p(), this%solution, mean_norm)   
          l1        = scalar_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_p(), this%solution, l1_norm)   
          l2        = scalar_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_p(), this%solution, l2_norm)   
          lp        = scalar_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_p(), this%solution, lp_norm)   
          linfty    = scalar_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_p(), this%solution, linfty_norm)   
          h1_s      = scalar_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_p(), this%solution, h1_seminorm) 
          h1        = scalar_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_p(), this%solution, h1_norm) 
          w1p_s     = scalar_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_p(), this%solution, w1p_seminorm)   
          w1p       = scalar_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_p(), this%solution, w1p_norm)   
          w1infty_s = scalar_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_p(), this%solution, w1infty_seminorm) 
          w1infty   = scalar_error_norm%compute(this%test_elasticity_analytical_functions%get_solution_function_p(), this%solution, w1infty_norm)  
          if ( this%par_environment%am_i_l1_root() ) then
             write(*,'(a12,a8)')      this%test_elasticity_integration%get_field_name(field_id), ' field: '
             write(*,'(a20,e32.25)') 'mean_norm:', mean              ; check ( abs(mean) < 1.0e-04 ) 
             write(*,'(a20,e32.25)') 'l1_norm:', l1                  ; check ( l1 < 1.0e-04 )        
  ...
  subroutine write_solution(this)
    ...
    if(this%test_params%get_write_solution()) then
       if ( this%par_environment%am_i_l1_task() ) then
        ...
        do field_id = 1, this%test_elasticity_integration%get_number_fields()
           name =  this%test_elasticity_integration%get_field_name(field_id)
 ...
 !========================================================================================
  ...
  subroutine free(this)
    ...
    call this%test_elasticity_integration%free()
  end subroutine free  
...
end module test_elasticity_driver_names

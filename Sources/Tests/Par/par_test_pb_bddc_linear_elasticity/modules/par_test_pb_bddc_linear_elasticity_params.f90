...
contains
  ...
  subroutine par_test_pb_bddc_linear_elasticity_params_define_parameters(this)
    ...
    error = list%set(key = struct_hex_triang_domain_limits_key        , value = [0.0_rp,2.0_rp,0.0_rp,0.5_rp,0.0_rp,0.5_rp]); check(error==0)
    error = list%set(key = is_a_beam_key                     , value =  .true.)                      ; check(error==0)
    ...
    error = switches%set(key = struct_hex_triang_domain_limits_key    , value = '--hex_mesh_domain_limits')   ; check(error==0)
    error = switches%set(key = is_a_beam_key                 , value = '--is_a_beam')                ; check(error==0)
    ...
    error = switches_ab%set(key = struct_hex_triang_domain_limits_key    , value = '-domain_limits')      ; check(error==0)
    error = switches_ab%set(key = is_a_beam_key                 , value = '-is_a_beam')          ; check(error==0)
    ...
    error = helpers%set(key = struct_hex_triang_domain_limits_key     , value  = 'Limits of the domain')      ; check(error==0)
    error = helpers%set(key = is_a_beam_key                  , value  = 'Is the studied object a beam')      ; check(error==0)
    ...
    error = required%set(key = is_a_beam_key                 , value = .false.) ; check(error==0)

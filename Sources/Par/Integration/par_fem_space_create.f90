...
contains
  ...
  !*********************************************************************************
  subroutine par_fe_space_create ( p_trian, dhand, p_femsp, problem, p_cond, continuity, order,     &
    ...
    if( p_femsp%p_trian%p_env%p_context%iam >= 0 ) then
       ...
       call fe_space_allocate_structures(  p_trian%f_trian, dhand, p_femsp%f_space,            &
...
!!$    check (istat == 0)    
       ...
       call fe_space_fe_list_create ( p_femsp%f_space, problem, which_approx, continuity, order, material, p_cond%f_conditions )
       ...
       !write(*,*) 'num_ghosts', num_ghosts
       !do ielem = p_femsp%g_trian%num_elems+1, p_femsp%g_trian%num_elems+num_ghosts
       !   call finite_element_fixed_info_write( p_femsp%g_trian%elems(ielem)%topology )
       !end do
    end if

  end subroutine par_fe_space_create
  
  subroutine par_fe_space_free ( p_femsp )
    implicit none
    ! Dummy arguments
    type(par_fe_space_t), intent(inout) :: p_femsp  
    ...
    if( p_femsp%p_trian%p_env%p_context%iam >= 0 ) then
       ...
       do ielem = p_femsp%p_trian%f_trian%num_elems+1, p_femsp%p_trian%f_trian%num_elems+p_femsp%p_trian%num_ghosts
          ...
          call finite_element_free_unpacked(p_femsp%f_space%lelem(ielem))
       end do
       call fe_space_free(p_femsp%f_space)
    end if
    
    nullify ( p_femsp%p_trian )
  end subroutine par_fe_space_free

  subroutine par_fe_space_print ( p_femsp )
    implicit none
    type(par_fe_space_t), intent(in) :: p_femsp  
  ...
  subroutine ghost_fe_list_create( p_femsp )
    implicit none
    type(par_fe_space_t), target, intent(inout) :: p_femsp
    ...
    do ielem = p_femsp%p_trian%f_trian%num_elems+1, p_femsp%p_trian%f_trian%num_elems+p_femsp%p_trian%num_ghosts
       ...
       do ivar=1,nvars
          ...
          if ( istat == new_index) then 
             ...
             call finite_element_fixed_info_create(p_femsp%f_space%lelem_info(pos_elinf),f_type,              &

module psi_penv_mod
  ...
  private :: psi_register_mpi_extras
...
contains
  ...
  subroutine  psi_register_mpi_extras(info)
    ...
    psb_mpi_long_integer = mpi_integer8
    
    ! A.F.M.: added in order to take care of real precision.
    !         Unlike PSBLAS 2.4.0, both real and double precision 
    !         in the same program are no longer possible
    psb_mpi_real = mpi_double_precision
    if (rp == 4) psb_mpi_real = mpi_real
  ...
  subroutine psb_init(ictxt,np,basectxt,ids)
    ...
    ! E character(len=20), parameter :: name='psb_init'
#if defined(SERIAL_MPI) 
    ictxt = nctxt
    nctxt = nctxt + 1
    ...
    if (present(np)) then 
      if (np < 1) then 
        ! E info=psb_err_initerror_neugh_procs_
        ! E call psb_errpush(info,name)
        ! E call psb_error()
        ictxt = mpi_comm_null
        return
      endif
  ...
  subroutine psb_exit(ictxt,close)
    ...
    logical  :: close_
    integer  :: info
    ! E character(len=20), parameter :: name='psb_exit'
  ...
  subroutine psi_damx_op(vin,vinout,len,itype)
    integer, intent(in)           :: len, itype
    real(rp), intent(in)    :: vin(len)
    real(rp), intent(inout) :: vinout(len)

...
use psi_comm_buffers_mod_names
  ...
  integer, private, parameter:: psb_int_tag      = 543987
  integer, private, parameter:: psb_double_tag   = psb_int_tag      + 1
  integer, private, parameter:: psb_logical_tag  = psb_double_tag   + 1
  integer, private, parameter:: psb_char_tag     = psb_logical_tag  + 1
  integer, private, parameter:: psb_int8_tag     = psb_char_tag     + 1  

  ! A.F.M.: the following tags could be useful for the future, when implementing
  !         our own nearest neighbours data exchange. I will leave them untouched
  !         for the time being.
  ...
  integer, parameter :: psb_double_swap_tag = psb_int_tag + psb_double_tag
  integer, parameter :: psb_int_swap_tag    = psb_double_swap_tag + 1 
...
use mpi
    ...
    integer(ip), intent(in)  :: dat
    integer, intent(in)  :: dst
    integer(ip), allocatable :: dat_(:)
    integer :: info 
...
use mpi
    ...
    ! do nothing
#else
    call mpi_recv(dat,1,psb_mpi_real,src,psb_double_tag,ictxt,status,info)
    call psb_test_nodes(psb_mesg_queue)
#endif    
...
#if defined(SERIAL_MPI) 
#else
    call mpi_recv(dat,size(dat),psb_mpi_real,src,psb_double_tag,ictxt,status,info)
    call psb_test_nodes(psb_mesg_queue)
#endif    
...
use mpi
    ...
    if (present(m)) then 
      ...
      ld = size(dat,1)
      n_ = size(dat,2)
      call mpi_type_vector(n_,m_,ld,psb_mpi_real,mp_rcv_type,info)
      if (info == mpi_success) call mpi_type_commit(mp_rcv_type,info)
      if (info == mpi_success) call mpi_recv(dat,1,mp_rcv_type,src,&
           & psb_double_tag,ictxt,status,info)
      if (info == mpi_success) call mpi_type_free(mp_rcv_type,info)
    else
      call mpi_recv(dat,size(dat),psb_mpi_real,src,&
           & psb_double_tag,ictxt,status,info)
    end if
    if (info /= mpi_success) then 
      write(0,*) 'Error in psb_recv', info

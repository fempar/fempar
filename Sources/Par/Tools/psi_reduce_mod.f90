...
use mpi
    ...
    if (root_ == -1) then 
      call mpi_allreduce(dat,dat_,1,psb_mpi_real,mpi_max,ictxt,info)
      dat = dat_
    else
      call mpi_reduce(dat,dat_,1,psb_mpi_real,mpi_max,root_,ictxt,info)
      dat = dat_
    endif
#endif    
...
use mpi
    ...
    if (root_ == -1) then 
      ...
      dat_ = dat
      if (info == psb_success_) &
           & call mpi_allreduce(dat_,dat,size(dat),psb_mpi_real,&
           & mpi_max,ictxt,info)
    else
      if (iam == root_) then 
        call psb_realloc(size(dat),dat_,info)
        dat_ = dat
        call mpi_reduce(dat_,dat,size(dat),psb_mpi_real,mpi_max,root_,ictxt,info)
      else
        call mpi_reduce(dat,dat_,size(dat),psb_mpi_real,mpi_max,root_,ictxt,info)
      end if
    endif
#endif    
...
use mpi
    ...
    if (root_ == -1) then 
      ...
      dat_ = dat
      if (info == psb_success_)&
           & call mpi_allreduce(dat_,dat,size(dat),psb_mpi_real,mpi_max,ictxt,info)
    else
      if (iam == root_) then 
        call psb_realloc(size(dat,1),size(dat,2),dat_,info)
        dat_ = dat
...
use mpi
    ...
    if (root_ == -1) then 
      call mpi_allreduce(dat,dat_,1,psb_mpi_real,mpi_min,ictxt,info)
      dat = dat_
    else
      call mpi_reduce(dat,dat_,1,psb_mpi_real,mpi_min,root_,ictxt,info)
      dat = dat_
    endif
#endif    
...
use mpi
    ...
    if (root_ == -1) then 
      ...
      if (iam == root_) then 
        call psb_realloc(size(dat),dat_,info)
        dat_ = dat
        call mpi_reduce(dat_,dat,size(dat),psb_mpi_real,mpi_min,root_,ictxt,info)
      else
        call mpi_reduce(dat,dat_,size(dat),psb_mpi_real,mpi_min,root_,ictxt,info)
      end if
    endif
#endif    
...
use mpi
    ...
    if (root_ == -1) then 
      ...
      dat_ = dat
      if (info == psb_success_)&
           & call mpi_allreduce(dat_,dat,size(dat),psb_mpi_real,mpi_min,ictxt,info)
    else
      if (iam == root_) then 
        call psb_realloc(size(dat,1),size(dat,2),dat_,info)
        dat_ = dat
...
use mpi
    ...
    if (root_ == -1) then 
      call mpi_allreduce(dat,dat_,1,psb_mpi_real,mpi_damx_op,ictxt,info)
      dat = dat_
    else
      call mpi_reduce(dat,dat_,1,psb_mpi_real,mpi_damx_op,root_,ictxt,info)
      dat = dat_
    endif
#endif    
...
use mpi
    ...
    if (root_ == -1) then 
      ...
      if (iam == root_) then 
        call psb_realloc(size(dat),dat_,info)
        dat_ = dat
        call mpi_reduce(dat_,dat,size(dat),psb_mpi_real,mpi_damx_op,root_,ictxt,info)
      else
        call mpi_reduce(dat,dat_,size(dat),psb_mpi_real,mpi_damx_op,root_,ictxt,info)
      end if
    endif
#endif    
...
use mpi
    ...
    if (root_ == -1) then 
      ...
      dat_ = dat
      if (info == psb_success_)&
           & call mpi_allreduce(dat_,dat,size(dat),psb_mpi_real,mpi_damx_op,ictxt,info)
    else
      if (iam == root_) then 
        call psb_realloc(size(dat,1),size(dat,2),dat_,info)
        dat_ = dat
...
use mpi
    ...
    if (root_ == -1) then 
      call mpi_allreduce(dat,dat_,1,psb_mpi_real,mpi_damn_op,ictxt,info)
      dat = dat_
    else
      call mpi_reduce(dat,dat_,1,psb_mpi_real,mpi_damn_op,root_,ictxt,info)
      dat = dat_
    endif
#endif    
...
use mpi
    ...
    if (root_ == -1) then 
      ...
      if (iam == root_) then 
        call psb_realloc(size(dat),dat_,info)
        dat_ = dat
        call mpi_reduce(dat_,dat,size(dat),psb_mpi_real,mpi_damn_op,root_,ictxt,info)
      else
        call mpi_reduce(dat,dat_,size(dat),psb_mpi_real,mpi_damn_op,root_,ictxt,info)
      end if
    endif
#endif    
...
use mpi
    ...
    if (root_ == -1) then 
      ...
      dat_ = dat
      if (info == psb_success_)&
           & call mpi_allreduce(dat_,dat,size(dat),psb_mpi_real,mpi_damn_op,ictxt,info)
    else
      if (iam == root_) then 
        call psb_realloc(size(dat,1),size(dat,2),dat_,info)
        dat_ = dat
...
use mpi
    ...
    if (root_ == -1) then 
      call mpi_allreduce(dat,dat_,1,psb_mpi_real,mpi_sum,ictxt,info)
      dat = dat_
    else
      call mpi_reduce(dat,dat_,1,psb_mpi_real,mpi_sum,root_,ictxt,info)
      dat = dat_
    endif
#endif    
...
use mpi
    ...
    if (root_ == -1) then 
      ...
      if (iam == root_) then 
        call psb_realloc(size(dat),dat_,info)
        dat_ = dat
        call mpi_reduce(dat_,dat,size(dat),psb_mpi_real,mpi_sum,root_,ictxt,info)
      else
        call mpi_reduce(dat,dat_,size(dat),psb_mpi_real,mpi_sum,root_,ictxt,info)
      end if
    endif
#endif    
...
use mpi
    ...
    if (root_ == -1) then 
      ...
      dat_ = dat
      if (info == psb_success_)&
           & call mpi_allreduce(dat_,dat,size(dat),psb_mpi_real,mpi_sum,ictxt,info)
    else
      if (iam == root_) then 
        call psb_realloc(size(dat,1),size(dat,2),dat_,info)
        dat_ = dat
...
use mpi
    ...
    if (root_ == -1) then 
      call mpi_allreduce(dat,dat_,1,psb_mpi_real,mpi_dnrm2_op,ictxt,info)
      dat = dat_
    else
      call mpi_reduce(dat,dat_,1,psb_mpi_real,mpi_dnrm2_op,root_,ictxt,info)
      dat = dat_
    endif
#endif    
...
use mpi
    ...
    if (root_ == -1) then 
      ...
      if (iam == root_) then 
        call psb_realloc(size(dat),dat_,info)
        dat_ = dat
        call mpi_reduce(dat_,dat,size(dat),psb_mpi_real,&
             & mpi_dnrm2_op,root_,ictxt,info)
      else
        call mpi_reduce(dat,dat_,size(dat),psb_mpi_real,&
             & mpi_dnrm2_op,root_,ictxt,info)
      end if
    endif
#endif    

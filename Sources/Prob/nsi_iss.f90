...
contains
  ...
  !=================================================================================================
  subroutine nsi_matvec(approx,start,finite_element)
    !----------------------------------------------------------------------------------------------!
    ...
    do inode=1,nnodu
       do idime=1,ndime
          do jnode=1,nnodu
             do jdime=1,ndime
                ! Block V-U
                finite_element%p_mat%a(start(idime)+inode-1,start(jdime)+jnode-1) =           &
                     & finite_element%p_mat%a(start(idime)+inode-1,start(jdime)+jnode-1) +    &
                     & elmat_vu(idime,jdime,inode,jnode)
             end do    
             ! Block V-U (diag)
             finite_element%p_mat%a(start(idime)+inode-1,start(idime)+jnode-1) =              &
                  & finite_element%p_mat%a(start(idime)+inode-1,start(idime)+jnode-1) +       &
                  & elmat_vu_diag(inode,jnode)
          end do
          do jnode=1,nnodp
             ! Block V-P
             finite_element%p_mat%a(start(idime)+inode-1,start(ndime+1)+jnode-1) =            &
                  & finite_element%p_mat%a(start(idime)+inode-1,start(ndime+1)+jnode-1) +     &
                  & elmat_vp(idime,1,inode,jnode)
             ! Block Q-U
             finite_element%p_mat%a(start(ndime+1)+jnode-1,start(idime)+inode-1) =            &
                  & finite_element%p_mat%a(start(ndime+1)+jnode-1,start(idime)+inode-1) +     &
                  & elmat_qu(1,idime,jnode,inode)
          end do
          ! Block U
          finite_element%p_vec%a(start(idime)+inode-1) = finite_element%p_vec%a(start(idime)+inode-1) + &
               & elvec_u(idime,inode)
       end do
    end do

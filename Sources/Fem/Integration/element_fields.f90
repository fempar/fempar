...
module element_fields_names
  ...
  type, abstract, extends(memory_guard) :: field
     contains
...
! Scalar
  ...
  subroutine scalar_assign(yfield,xfield) ! y=x
    implicit none
    class(scalar), intent(inout) :: yfield
    class(field) , intent(in)    :: xfield
    integer(ip)  :: ng
  ...
  function product_scalar_field(xfield,yfield) result(zfield)
    implicit none
    class(scalar), intent(in)  :: xfield     ! xfield(ngaus)
    class(field) , intent(in)  :: yfield     ! yfield
    class(field) , allocatable :: zfield     ! zfield
...
! Vector
  ...
  subroutine vector_assign(yfield,xfield) ! y=x
    implicit none
    class(vector), intent(inout) :: yfield
    class(field) , intent(in)    :: xfield
    integer(ip)  :: n1,ng
    call xfield%GuardTemp()
    if(allocated(yfield%a)) call memfree(yfield%a,__FILE__,__LINE__)
  ...
  function product_vector_field(xfield,yfield) result(zfield)
    implicit none
    class(vector), intent(in)  :: xfield    ! xfield(ngaus)
    class(field) , intent(in)  :: yfield     ! yfield
    class(field) , allocatable :: zfield     ! zfield
    integer(ip)   :: i,j,k,n1,n2,ng
    call xfield%GuardTemp()
...
! Tensor
  ...
  subroutine tensor_assign(yfield,xfield) ! y=x
    implicit none
    class(tensor), intent(inout) :: yfield
    class(field) , intent(in)    :: xfield
    integer(ip)  :: n1,n2,ng
    call xfield%GuardTemp()
    if(allocated(yfield%a)) call memfree(yfield%a,__FILE__,__LINE__)
  ...
  function product_tensor_field(xfield,yfield) result(zfield)
    implicit none
    class(tensor), intent(in)  :: xfield    ! xfield(ngaus)
    class(field) , intent(in)  :: yfield     ! yfield
    class(field) , allocatable :: zfield     ! zfield
    integer(ip)   :: i,j,k,n1,n2,ng
    call xfield%GuardTemp()
